...
export default function ManageStudents() {
  ...
  const mutation = useMutation({
	mutationFn: async (data: typeof formData) => {
	  const endpoint = action === "add" ? `${import.meta.env.VITE_BACKEND_URL}/students/add` : `${import.meta.env.VITE_BACKEND_URL}/students/remove`;
	...
	onSuccess: () => {
	  toast({
		title: "Success",
		description: `Student ${action === "add" ? "added" : "removed"} successfully!`,
		variant: "success",
		duration: 2000,
	  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (
      (action === "add" &&
        (!formData.team || !formData.firstName || !formData.lastName || !formData.computingID)) ||
  ...
  return (
	<div className="bg-background text-foreground flex flex-col items-center py-12">
	  <Card className="w-full max-w-2xl p-6">
		<Label htmlFor={action === "add" ? "addStudentForm" : "removeStudentForm"} className="block text-lg font-semibold mb-4">
		  {action === "add" ? "Add Student" : "Remove Student"}
		</Label>
		<form onSubmit={handleSubmit} id={action === "add" ? "addStudentForm" : "removeStudentForm"}>
		  {action === "add" && (
			<>
